...
sequenceDiagram
    ...
    Save->>Save: atomicWriteFile (temp→rename)
    Provider->>WebView: saveResult (success)
```

## Provider Lifecycle

### `HwpEditorProvider` (`src/provider/hwp/HwpEditorProvider.ts` — 316 lines)
...
This prevents partial writes: if the process crashes mid-write, only the `.tmp-*` file is corrupted; the original file is untouched.
...
### `hwpMessageSchema.ts` (`src/common/hwpMessageSchema.ts` — 166 lines)
...
### `Hwp.tsx` (`src/react/view/hwp/Hwp.tsx` — ~280 lines)

State machine:

```
LOADING → (fileData) → EDITING → (Ctrl+S) → SAVING → EDITING
                                    ↓
                          (export fail) → ERROR → EDITING (retry)
```
...
### `createSecureRhwpEditor.ts` (`src/react/view/hwp/rhwpBridge/createSecureRhwpEditor.ts` — 463 lines)
...
iframe.srcdoc = studioHtml (patched by build.ts)
       ↓
window.__rhwpBridge = {ready, loadFile, pageCount, getPageSvg, exportHwp, exportHwpx}
       ↓
